...
- **AC14 — Consistent GetOrder read under a concurrent close (BR8):** Given a `GetOrder` request running concurrently with a `CloseOrder` request against the same order, when both complete, then the `GetOrder` response reflects either the complete pre-close state (`Status: Open`) or the complete post-close state — it is never `Status: Open` for a response that, by the time it is sent, describes an order that has already closed. (`GetOrder` will not observe the order at all — a 404 — once the `Closed` filter in `GetOpenByTableAsync` excludes it; this criterion is about the boundary moment where the two requests genuinely overlap.)
...
`OrderId` is a resource identifier in the request path (`POST /api/v1/restaurants/orders/{orderId:guid}/close`) — the same `:guid`-constrained route pattern `add-item.md` already uses for its `/{orderId:guid}/items` route, so a malformed value fails route matching and surfaces as 404 (the same as a well-formed but nonexistent `OrderId`) rather than reaching the handler as a 400. An unconstrained `{orderId}` segment would instead let minimal-API model binding turn a malformed value into a 400, which would contradict the 404 this table requires — the `:guid` constraint is what makes this contract implementable.
