...
- **Integration tests:** AC1–AC13 above, executed against the real API and database, including:
  ...
  - **Validation (AC2's malformed-`OrderId` case):** a malformed `OrderId` is rejected as 404, following the same pattern as the existing `AddItem` endpoint tests for its own `{orderId:guid}` route.
