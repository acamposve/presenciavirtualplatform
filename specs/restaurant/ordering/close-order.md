...
`GetOrder` deliberately does not appear in this diagram: per its own specification, it is a read-only query that "can be exercised at any point after `CreateOrder` and interleaved freely with `AddItem` calls — it does not advance the order lifecycle" (`get-order.md`). It remains callable after an order is closed, but per `get-order.md` FR3/AC2 it will reject that request as Not Found, the same as for any table with no `Open` order — it does not somehow keep returning a closed order under a different status.
...
- **Consistency:** the closed status must be immediately visible to a subsequent read (`GetOrder` will no longer return the order, per `get-order.md` FR3; a direct read by `OrderId`, if any future capability adds one, must reflect `Closed`). See also BR8: this specification is the one `get-order.md` BR4 anticipated would need to revisit `GetOrder`'s own read consistency.
...
- **BR3:** An `Idempotency-Key`, when supplied, uniquely identifies a single logical close-order attempt within a tenant — scoped by `(TenantId, IdempotencyKey)` alone, not also by `OrderId`, for the same reason `add-item.md` BR6 gives (a key scoped by `(TenantId, OrderId, IdempotencyKey)` could not detect reuse against a different `OrderId` as a conflict). The stored record retains the original `OrderId`, so a replay (same `OrderId`) can be told apart from a conflicting reuse (a different `OrderId`). A replay MUST NOT attempt to transition the order's status again; the response reflects the order's **current** state at the time of the replay, not a frozen snapshot of the original close.
  ...
  - When an `Idempotency-Key` is supplied, the existing claim for it (if any) MUST be checked **before** resolving whether the requested `OrderId` exists, mirroring `AddItemHandler`'s established precedence for the identical reason: a key reused against a different, possibly nonexistent, `OrderId` must deterministically return 409 Conflict (AC7), not 404 by resolving the (nonexistent) order first and never reaching the conflict check.
...
- **BR8 (resolves a question `get-order.md` deliberately deferred):** `get-order.md` BR4 explicitly left its order-row read and its items read as two independent queries, reasoning that nothing in the system could yet mutate an order's own row after creation — and said that "a future specification (e.g. `CloseOrder`, `CancelOrder`) [that] introduces a way to mutate the order's own row after creation ... MUST then decide whether `GetOrder` needs a single consistent read." This is that specification, and the answer is yes: `GetOrder`'s order-row read (`OrderRepository.GetOpenByTableAsync`) and its items read MUST be combined into a single consistent read (e.g. one SQL statement covering both, or a `REPEATABLE READ`/`SERIALIZABLE` transaction spanning both, per the same isolation reasoning `get-order.md`'s own review already worked through) — otherwise a `GetOrder` request that reads the order row just before a concurrent `CloseOrder` commits, then reads items just after, could return a response claiming `Status: Open` for an order that, by the time the response is sent, is already `Closed`.
...
- **AC14 — Consistent GetOrder read under a concurrent close (BR8):** Given a `GetOrder` request whose database read overlaps in time with a `CloseOrder` request's transition against the same order, when the `GetOrder` request completes, then its outcome corresponds to some single point in the close's commit sequence. Because `GetOrder` only ever returns orders with `status = 'Open'` (`get-order.md` FR3/AC2 — it has no representation for a `Closed` order; that belongs to `CloseOrder`'s or a future direct-by-`OrderId` query's response), the only two valid outcomes are: (a) a complete pre-close snapshot — `Status: Open` with items/`Total` as of some point before the close committed, including any `AddItem` that itself committed before that point — or (b) `404 Not Found`, once the close has committed and is visible to the read. What this forbids is a response no single point in time could have produced — e.g. `Status: Open` combined with a line that was only added after the close committed. This is a property of the database read's isolation, not of when the HTTP response happens to reach the client — a pre-close snapshot is valid even if transmitted after the close commits elsewhere.
...
The first two rows can overlap for a request carrying an `Idempotency-Key` that was already used for a *different* `OrderId`: per BR3, the claim check runs before order resolution, so that case always returns 409 Conflict, never 404, regardless of whether the requested `OrderId` happens to exist.
...
- This new table is tenant-owned data like every other table in this module: its migration MUST enable and force Row-Level Security with a tenant-scoped policy (covering both the read and the `WITH CHECK` insert side), and grant the least-privilege application role `SELECT`/`INSERT` on it — following the exact pattern the `0003`/`0009` migrations already established for `order_idempotency_keys`/`add_item_idempotency_keys`. Skipping this would either make key lookups inaccessible to the app role or, worse, leave them unenforced across tenants.
...
- **Integration tests:** AC1–AC14 above, executed against the real API and database, including:
  ...
  - **Consistent GetOrder read under a concurrent close (BR8, AC14):** with `CloseOrder`'s transaction deliberately controlled to commit at a specific point relative to a `GetOrder` request's own reads, the `GetOrder` response must reflect either the complete pre-close or complete post-close state, never a mix — proving `GetOrder`'s order-row and items reads were combined into a single consistent read, not left as the two independent queries `get-order.md` originally shipped with.
