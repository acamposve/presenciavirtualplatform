...
- **AC14 — Consistent GetOrder read under a concurrent close (BR8):** Given a `GetOrder` request whose database read overlaps in time with a `CloseOrder` request's transition against the same order, when the `GetOrder` request completes, then its response corresponds to some single point in the close's commit sequence — either entirely as of a point *before* the close committed (`Status: Open`, with items/`Total` as of that same point), or entirely as of a point *after* (whatever `Status`/items/`Total` hold once the close, and anything that committed before it, are fully applied). What this forbids is a response no single point in time could have produced — e.g. `Status: Open` combined with a line that was only added after the close committed, or `Status: Closed` combined with items from before an `AddItem` that the close's own commit ordering implies should already be included. This is a property of the database read's isolation, not of when the HTTP response happens to reach the client — a snapshot taken before the close commits is a valid pre-close response even if it is transmitted afterward.
...
- The `OrderItem` rows read to build the response (FR7) MUST be read within the same order-scoped lock/transaction as the status transition (BR7) — e.g. re-read after the `UPDATE` commits, while still holding the lock — not from an earlier, separate read taken before the lock was acquired. Otherwise a concurrent `AddItem` that legitimately commits before the close (AC13's "both succeed" case) could be silently missing from the very response that is supposed to reflect the order's final state.
