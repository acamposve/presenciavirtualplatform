...
This specification follows `constitution.md` Article I. It requires human review and approval before implementation begins, per Article 1.4. Approved after four rounds of review (business rules, tenant-consistency, and concurrency semantics for BR4/BR6/BR7); implementation is in [PR #9](https://github.com/acamposve/presenciavirtualplatform/pull/9).
...
8. The system MUST support an optional `Idempotency-Key` request header, with the same semantics as `CreateOrder`'s (FR7/BR6 in `create-order.md`): a repeated request with the same key does not re-apply the mutation (does not add the item, or increase the line's quantity, a second time). The response to that repeated request still reflects the order's *current* state (FR7), not a frozen snapshot of the response given at the original call — see BR6.
...
- **BR6:** An `Idempotency-Key`, when supplied, uniquely identifies a single logical add-item attempt **within a tenant** — scoped by `(TenantId, IdempotencyKey)` alone, *not* also by `OrderId`. (A key scoped by `(TenantId, OrderId, IdempotencyKey)` would fail AC9: reusing the same key against a different `OrderId` would look like a key never seen before for that order, and would be processed as a new request instead of rejected as a conflict.) The stored record retains only the original `OrderId`, `MenuItemId`, and `Quantity` — enough to tell a replay (all three match) apart from a conflicting reuse (any of the three differs), the same pattern `create-order.md`'s `IdempotencyRecord` already uses for `TableId`. It is **not** a snapshot of the response: a replay MUST NOT re-apply the mutation (must not add the item, or increase the line's quantity, a second time), but the response returned for a replay is the order's **current** state, re-read at the time of the replay — which may include other changes made to the order since the original call — not a frozen copy of what was returned originally (see FR7/FR8). Reusing a key with a different request (different `OrderId`, `MenuItemId`, or `Quantity`) MUST be rejected rather than silently accepted.
  - Claiming the key and applying the mutation (BR4) MUST commit as a single atomic transaction. If two concurrent requests supply the same key, at most one may successfully claim it *and* apply its mutation; the other MUST roll back without applying any mutation of its own, then re-read the order's current state to respond (or reject as a conflict, if its own request differs from the winner's) — the same recovery pattern `create-order.md` already uses (`IdempotencyKeyRaceLostException`) rather than treating "record the key" and "apply the change" as two independent writes, which would let two concurrent identical requests both mutate before either claims the key.
- **BR7:** A menu item MAY be marked `IsAlcoholic`. A tenant MAY configure a maximum quantity per line for alcoholic items (`MaxAlcoholicItemQuantityPerLine`). When configured, the *resulting* quantity of an alcoholic item's line (after merging, per BR4) MUST NOT exceed it. When not configured for a tenant, no limit is enforced. The specific limit is a business/legal decision for each tenant to set, not a value this specification fixes.
  - This check MUST be evaluated atomically together with the quantity update, not as a separate read-then-decide step ahead of BR4's atomic upsert — e.g. by locking the line's row (existing or about-to-be-created) within the same transaction before evaluating the limit and applying the change. Without this, two concurrent requests can each read the same pre-update quantity, each independently pass the limit check against it, and jointly commit a quantity that exceeds the limit even though neither request alone would have. Two concurrent requests against the same alcoholic line MUST be serialized so that at most one can bring the quantity up to the limit; the other, once serialized behind it, MUST be rejected if it would exceed the limit.
...
- **AC8 — Idempotent replay:** Given a prior successful add-item request with `Idempotency-Key: K`, when the same request (same `OrderId`, `MenuItemId`, `Quantity`) is repeated with the same key, then the item is not added again (the line's quantity does not increase a second time), and the response reflects the order's current state at the time of the replay — including any unrelated changes made to the order since the original call, not a frozen copy of the original response.
...
- **Writes:** an `OrderItem` row is created or its `Quantity` increased (BR4). The Order's `Total` MUST always be derived from its current `OrderItem` rows (e.g. a `SUM` computed at read time) — it MUST NOT be maintained as an independently incremented column updated outside the same transaction as the item mutation. An incremental `Total` update racing separately from BR4's atomic quantity upsert can lose an update and violate BR5 even when the quantity itself is updated correctly; deriving it removes that failure mode entirely rather than requiring its own concurrency-safe arithmetic.
...
- **Integration tests:** AC1–AC15 above, executed against the real API and database, including:
  ...
  - **Concurrent merge (BR4):** two simultaneous `AddItem` requests for the same order and menu item must both be reflected in the final quantity (e.g. two concurrent requests for quantity 1 each must result in a line at quantity 2, not 1) — proving the atomic upsert, not a read-then-write, is what's implemented. The order's `Total` after both requests complete must also reflect both, proving `Total` is derived rather than independently (and unsafely) incremented.
  - **Concurrent identical idempotent requests (BR6):** two simultaneous `AddItem` requests carrying the *same* `Idempotency-Key` and the same body must result in the item being added exactly once — proving the idempotency claim and the mutation commit together, not as two independent writes that could both apply before either claims the key.
  - **Concurrent requests against an alcoholic limit (BR7):** given a configured limit N and a line already one below it, two simultaneous requests that would each individually bring it to the limit (but together would exceed it) must result in exactly one succeeding and the other rejected as Conflict — the final quantity must never exceed N. This proves the limit check is serialized with the update, not a separate check performed before it.
