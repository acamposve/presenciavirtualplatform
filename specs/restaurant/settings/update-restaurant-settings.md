...
- **Tenant isolation (ADR 0002 rule 8 — both read and write):**
  - **Read:** `restaurant.settings` has had RLS enabled since `add-item.md`, and `AddItemHandler` already reads it through the least-privileged application role — but no existing test queries it directly through that role (`TestRestaurantSettingsSeeder` writes through the admin/migration connection, which bypasses RLS as a superuser and so proves nothing about it). Add a direct test: a settings row created for Tenant A is visible when queried through `presenciavirtual_app` with Tenant A's context set, and invisible (querying by `tenant_id` alone, no application-level filter) with Tenant B's context set instead.
