...
- `src/` follows the Modular Monolith structure: `src/Api` (the ASP.NET Core host) and `src/Modules/<Context>` (one folder per bounded context, e.g. `Core`, `Restaurant`), each with a main project, an `.Infrastructure` project, and a `.Tests` project. A module's project is only created once its first specification is approved — its internal Vertical Slice organization is not duplicated here to avoid documentation drift; see `architecture.md` Sections 4, 7 and 9 for the target shape.
